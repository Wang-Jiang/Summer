...
### @After注解
@Before是在路由方法执行之前调用的拦截器，@After则是在路由方法直接之后调用的拦截器。一般来说，路由方法执行之后，还需要拦截器的情况不是很多，但是如果仅仅有Before，AOP特性不够完整，因此还是补充了After的功能。一个非常典型的用法，需要记录整个路由的执行时间，在路由调用之前，获取当前时间，放到Attribute中，当方法执行完成之后，获取当前时间，相减一下就知道路由方法的执行时间了
